package com.github.bjlhx15.security.cert;
...
public class CertificateCoder {
	...
	private static PrivateKey getPrivateKey(String keyStorePath, String alias, String password) throws Exception {
		KeyStore ks = CertificateCoder.getKeyStore(keyStorePath, password);
...
//	public KeyPair
	...
	private static KeyStore getKeyStore(String keyStorePath, String password) throws Exception {
		...
		KeyStore ks = KeyStore.getInstance(CertificateCoder.KEY_STORE);
	...
	public static boolean verifyCertificate(String certificatePath) {
		return CertificateCoder.verifyCertificate(new Date(), certificatePath);
	}
	...
	public static String sign(byte[] sign, String keyStorePath, String alias, String password) throws Exception {
		// 获得证书
		X509Certificate x509Certificate = (X509Certificate) CertificateCoder.getCertificate(keyStorePath, alias,
				password);
		// 获取私钥
		KeyStore ks = CertificateCoder.getKeyStore(keyStorePath, password);
		// 取得私钥
		...
		return CertificateCoder.encryptBASE64(signature.sign());
	}
	...
	public static boolean verify(byte[] data, String sign, String certificatePath) throws Exception {
		// 获得证书
		X509Certificate x509Certificate = (X509Certificate) CertificateCoder.getCertificate(certificatePath);
		// 获得公钥
		...
		return signature.verify(CertificateCoder.decryptBASE64(sign));
	...
	public static boolean verifyCertificate(Date date, String keyStorePath, String alias, String password) {
		boolean status = true;
		try {
			Certificate certificate = CertificateCoder.getCertificate(keyStorePath, alias, password);
			status = CertificateCoder.verifyCertificate(date, certificate);
		} catch (Exception e) {
			status = false;
		}
	...
	public static boolean verifyCertificate(String keyStorePath, String alias, String password) {
		return CertificateCoder.verifyCertificate(new Date(), keyStorePath, alias, password);
	}
